        ...
        <View style={{...containerStyle}}>
            <View style={{position:'absolute', left:SIZES.padding, top:0, bottom:0, justifyContent:'space-between'}}> 
                {getYAxisLabelValues().map((item,index) => {
                    return(
                        <Text style={{color:COLORS.lightGray3, ...FONTS.h3}} key={index}>{item}</Text>
                    )
                })}
            </View>
            {data.lenght > 0 && 
            <ChartPathProvider data={{points, smoothingStrategy: 'bezier'}}>
                ...
                <ChartDot>
                    ...
                    <View style={{width:15, height:15, borderRadius:10, backgroundColor:COLORS.lightGreen}} /></View>
                    <ChartYLabel format={formatUSD} style={{color: COLORS.white, ...FONTS.body5}} />
                    <ChartXLabel format={formateDateTime} style={{marginTop:3, color:COLORS.lightGray3, ...FONTS.body5, lineHeight: 15}} />
                    </View>
                </ChartDot>
            </ChartPathProvider>
